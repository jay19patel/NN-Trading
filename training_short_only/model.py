...
class TradingLoss(nn.Module):
    ...
    def forward(self, predictions: Dict[str, torch.Tensor], targets: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        per_sample_pnl_effect = ((actual_pnl < 0).float() * actual_pnl.abs() * pred_qty - (actual_pnl > 0).float() * actual_pnl.abs() * (1.0 - pred_qty))
        ...
        total = (self.alpha * signal_loss + self.beta * sizing_loss + self.gamma * pnl_effect + self.consistency_weight * consistency_loss)
        return {"total": total, "signal_loss": signal_loss, "sizing_loss": sizing_loss, "pnl_effect": pnl_effect, "consistency_loss": consistency_loss}
