...
from neural_engine.backtest_short import run_short_backtest
...
def _threshold_search_for_side(y_true: np.ndarray, y_tp: np.ndarray, y_sl: np.ndarray, probs: np.ndarray, side_label: int) -> dict:
    ...
    rows = []
    min_trades = 3 # Lowered from 5 to be more inclusive during validation
    
    # Expanded search range to find the "sweet spot"
    for threshold in np.arange(0.30, 0.56, 0.025):
        ...
        fee_drag = float(trade_count * (config.strategy.ROUND_TRIP_FEE_PCT + 2.0 * config.strategy.SLIPPAGE_PCT))
        
        net_expectancy_pct = (gross_win - gross_loss - fee_drag) / max(trade_count, 1)
        profit_factor = gross_win / max(gross_loss + fee_drag, 1e-9)
        win_rate = float(wins.sum() / trade_count)
        ...
        # Scoring logic: prefer expectancy but require a minimum trade count
        sample_penalty = min(trade_count / 10.0, 1.0) # Penalty for very few trades
        score = net_expectancy_pct * sample_penalty
        
        if trade_count < min_trades:
            score -= 1.0 # Significant penalty for under-representation
...
def train_short_model():
    ...
    # Stricter weight decay for better calibration
    ...
    with get_progress() as progress:
        ...
        for epoch in range(config.training.EPOCHS):
            ...
            if (epoch + 1) % 1 == 0:
                console.print(
                    f"Epoch {epoch + 1:02d}: train_loss={avg_train_loss:.5f}, "
